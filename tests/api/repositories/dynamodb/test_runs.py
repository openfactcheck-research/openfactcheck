...
@pytest_asyncio.fixture(loop_scope="function")
async def workspace_id(dynamo_table: str, project_id: str) -> str:
    repo = DynamoWorkspaceRepository(dynamo_table, region_name=REGION)
...
@pytest.fixture
def repo(dynamo_table: str) -> DynamoRunRepository:
    return DynamoRunRepository(dynamo_table, region_name=REGION)


async def test_create(repo: DynamoRunRepository, project_id: str, workspace_id: str) -> None:
...
async def test_get(repo: DynamoRunRepository, project_id: str, workspace_id: str) -> None:
...
async def test_get_wrong_user(repo: DynamoRunRepository, project_id: str, workspace_id: str) -> None:
    created = await repo.create(USER_ID, project_id, RunCreate(workspace_id=workspace_id, pipeline=SAMPLE_PIPELINE))
...
async def test_get_nonexistent(repo: DynamoRunRepository, project_id: str) -> None:
    result = await repo.get(USER_ID, project_id, "does-not-exist")

    assert result is None


async def test_update_status_running(repo: DynamoRunRepository, project_id: str, workspace_id: str) -> None:
    created = await repo.create(USER_ID, project_id, RunCreate(workspace_id=workspace_id, pipeline=SAMPLE_PIPELINE))
...
async def test_update_status_completed(repo: DynamoRunRepository, project_id: str, workspace_id: str) -> None:
    created = await repo.create(USER_ID, project_id, RunCreate(workspace_id=workspace_id, pipeline=SAMPLE_PIPELINE))
...
async def test_update_status_failed(repo: DynamoRunRepository, project_id: str, workspace_id: str) -> None:
    created = await repo.create(USER_ID, project_id, RunCreate(workspace_id=workspace_id, pipeline=SAMPLE_PIPELINE))
...
async def test_update_status_nonexistent(repo: DynamoRunRepository, project_id: str) -> None:
    result = await repo.update_status(USER_ID, project_id, "does-not-exist", RunStatus.RUNNING)

    assert result is None


async def test_list_by_project(repo: DynamoRunRepository, project_id: str, workspace_id: str) -> None:
...
async def test_list_by_project_empty(repo: DynamoRunRepository, project_id: str) -> None:
    runs = await repo.list_by_project(USER_ID, project_id)
...
async def test_list_by_project_filters_by_workspace(
    repo: DynamoRunRepository, project_id: str, workspace_id: str, dynamo_table: str
) -> None:
    ws_repo = DynamoWorkspaceRepository(dynamo_table, region_name=REGION)
