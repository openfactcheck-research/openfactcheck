"""Tests for FactcheckGPTRetriever. The Serper client is faked."""
...
def test_FactcheckGPTRetriever_satisfies_protocol() -> None:
    assert isinstance(FactcheckGPTRetriever(serper=_FakeSerper([])), Retriever)


@pytest.mark.asyncio(loop_scope="function")
async def test_FactcheckGPTRetriever_collects_snippets_into_sources() -> None:
    responses = [
    ...
    retriever = FactcheckGPTRetriever(serper=serper, num_results=5)
    claim = Claim(text="c")
...
async def test_FactcheckGPTRetriever_no_questions_skips_search() -> None:
    serper = _FakeSerper([])
    retriever = FactcheckGPTRetriever(serper=serper)
    claim = Claim(text="c")
