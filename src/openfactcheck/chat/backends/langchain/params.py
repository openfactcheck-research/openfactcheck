"""Map our ``ModelConfig`` + ``RuntimeConfig`` to LangChain constructor kwargs."""
...
"""Constructor kwargs for LangChain chat model classes.

``Any`` is intentional: the dict is spread into framework constructors
(``cls(**kwargs)``) whose params have many specific types. A stricter
value type would force explicit casts at every call site.
"""
...
def _openai_kwargs(config: OpenAIConfig) -> Kwargs:
    ...
    if config.reasoning_effort is not None:
        params["model_kwargs"] = {"reasoning_effort": config.reasoning_effort}
    return params
...
def config_to_kwargs(config: ModelConfig, runtime: RuntimeConfig) -> Kwargs:
    """Translate our typed config to the dict kwargs LangChain constructors accept."""
