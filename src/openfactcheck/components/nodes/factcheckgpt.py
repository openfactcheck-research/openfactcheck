...
def aggregator(
    g: AnyGraphBuilder,
    *,
    node_id: str = "factcheckgpt/aggregator",
) -> Step[list[Verdict], Result, Any, Any]:
    """Build FactcheckGPT's aggregator and lift it onto ``g`` as a node: collected verdicts in, a result out.
    ...
    Returns:
        The registered node, ready to wire with [`edge_from`][openfactcheck.graph.GraphBuilder.edge_from].
    """
    component = FactcheckGPTAggregator()
...
def build_graph(*, chat: ChatClient, serper: SerperClient | None = None) -> Graph[Input, Result, None, None]:
    ...
    consolidating the per-claim verdicts into a result at the end. The reviser is not part of the graph; the
    facade applies it.
    ...
    """
    g: GraphBuilder[Input, Result, None, None] = GraphBuilder(input_type=Input, output_type=Result, name="factcheckgpt")
    g.add(
        *chain(
            g,
            g.start_node,
